...
func (r *Resource) ApplyCreateChange(ctx context.Context, obj, createChange interface{}) error {
	...
	for _, bucketInput := range createBucketsState {
...

		{
			...
			_, err = cc.Client.AWS.S3.CreateBucket(i)
		...
		if r.includeTags {
			...
			_, err = cc.Client.AWS.S3.PutBucketTagging(i)
			if err != nil {
				return microerror.Mask(err)
			}
		}
		...
		if bucketInput.IsLoggingBucket {
			...
			_, err = cc.Client.AWS.S3.PutBucketAcl(i)
			if err != nil {
				return microerror.Mask(err)
			}
		}
		...
		if bucketInput.IsLoggingBucket {
			...
			_, err = cc.Client.AWS.S3.PutBucketLifecycleConfiguration(i)
			if err != nil {
				return microerror.Mask(err)
			}
		}
		...
		if bucketInput.IsLoggingEnabled {
			...
			_, err = cc.Client.AWS.S3.PutBucketLogging(i)
			if err != nil {
				return microerror.Mask(err)
			}
		}

		{
			...
			_, err = cc.Client.AWS.S3.PutBucketEncryption(i)
			if err != nil {
				return microerror.Mask(err)
			}
		}
