...
func (r *Resource) ApplyDeleteChange(ctx context.Context, obj, deleteChange interface{}) error {
	bucketsInput, err := toTablesState(deleteChange)
	if err != nil {
		...
		g.Go(func() error {
			...
			for {
				var objects []*s3.ObjectIdentifier
				{
					...
					o, err := cc.Client.AWS.S3.ListObjectsV2(i)
					if err != nil {
						return microerror.Mask(err)
					}
					if len(o.Contents) == 0 {
...

				{
					...
					_, err := cc.Client.AWS.S3.DeleteObjects(i)
					if err != nil {
						return microerror.Mask(err)
					}
...

			{
				...
				_, err = cc.Client.AWS.S3.DeleteBucket(i)
				if err != nil {
					return microerror.Mask(err)
				}
			}
...
func (r *Resource) newDeleteChange(ctx context.Context, obj, currentState, desiredState interface{}) (interface{}, error) {
	...
	var bucketsToDelete []TableState
	...
	for _, bucket := range currentBuckets {
		...
		if r.canBeDeleted(bucket) && containsTableState(bucket.Name, desiredBuckets) {
